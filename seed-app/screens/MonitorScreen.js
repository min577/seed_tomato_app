...
const API_BASE_URL = 'http://seedfarm.co.kr:5678/webhook';
const RASPBERRY_URL = 'http://192.168.49.219:8080'; // 라즈베리파이 서버
const SNAPSHOT_URL = `${RASPBERRY_URL}/snapshot`; // 스냅샷 엔드포인트
...
function LiveMonitorTab() {
  ...
  const [autoAnalysisInterval, setAutoAnalysisInterval] = useState(30); // 기본 30초
  ...
  const addLog = (type, message, data = null) => {
    ...
    setAnalysisLogs(prev => [newLog, ...prev].slice(0, 50)); // 최대 50개 유지
  };

  // 실시간 데이터 조회
  const fetchRealtimeData = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/data-realtime`);
  ...
  const checkServerStatus = async () => {
    try {
      ...
      if (data.streaming) {
        ...
        await fetch(`${RASPBERRY_URL}/start`);
        setIsConnected(true);
        return true;
      }
    } catch (error) {
  ...
  // YOLO 분석 실행 (자동/수동 공용)
  // n8n camera-capture API를 통해 라즈베리파이 촬영 + YOLO 분석
  const runYoloAnalysis = async (isAuto = false) => {
    ...
    try {
      ...
      // 분석 완료까지 대기 (라즈베리파이 → n8n → YOLO)
      await new Promise(resolve => setTimeout(resolve, 3000));
      
      // 최신 데이터 조회
      const dataResponse = await fetch(`${API_BASE_URL}/data-realtime`);
...
function AnalyzeTab() {
  ...
  const runDiseaseAnalysis = async () => {
    ...
    try {
      ...
      console.log('📤 API 호출:', `${API_BASE_URL}/disease-diagnosis`);
      
      // 새 워크플로우 호출 (JSON 형식)
      const response = await fetch(`${API_BASE_URL}/disease-diagnosis`, {
        method: 'POST',
        headers: {
  ...
  // 건강 상태 표시 (서버에서 받은 값 또는 텍스트 파싱)
  const getHealthStatus = () => {
    if (!diagnosisResult) return null;
    
    // 서버에서 직접 받은 healthStatus 사용
