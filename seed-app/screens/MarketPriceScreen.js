...
const API_BASE = 'http://seedfarm.co.kr:5678/webhook';
...
const PriceCompareTab = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // ⭐ 캐시 방지: timestamp 파라미터 추가
      const timestamp = Date.now();
      const res = await fetch(`${API_BASE}/price-compare?_t=${timestamp}`, {
        headers: {
...
const PriceHistoryTab = () => {
  ...
  const fetchData = async (days) => {
    ...
    try {
      ...
      // ⭐ 캐시 방지
      const timestamp = Date.now();
      const res = await fetch(`${API_BASE}/price-history?start=${startStr}&end=${endStr}&_t=${timestamp}`, {
        headers: {
          'Cache-Control': 'no-cache',
        },
      });
